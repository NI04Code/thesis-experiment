...
OUTPUT_DIR="$EXPERIMENT_ROOT/results/resource-defragmentation/$LOAD_PATTERN/$RESOURCE/$SYSTEM/repeat-$REPEAT/$TIMESTAMP"
mkdir -p "$OUTPUT_DIR"
...
"$SHARED_ROOT/scripts/preflight.sh" | tee "$OUTPUT_DIR/preflight.txt"
"$SHARED_ROOT/scripts/setup-layout.sh" | tee "$OUTPUT_DIR/setup.log"
...
"$SHARED_ROOT/scripts/snapshot.sh" before "$OUTPUT_DIR"
python3 "$SHARED_ROOT/scripts/watch-pods.py" --namespace "$NS" --output "$OUTPUT_DIR/pod-lifecycle.tsv" &
WATCH_PID=$!
...
k6 run --out "json=$OUTPUT_DIR/api-load.json" \
  ...
  "$SHARED_ROOT/k6/api-load.js" > "$OUTPUT_DIR/api-load.log" 2>&1 &
API_PID=$!
...
python3 "$SHARED_ROOT/scripts/wait-threshold.py" \
  --namespace "$NS" \
  --pod "$api_pod" \
  --resource "$RESOURCE" \
  --threshold "$threshold" \
...
"$SHARED_ROOT/scripts/snapshot.sh" event "$OUTPUT_DIR"

log "running system event for ${SYSTEM}"
case "$SYSTEM" in
  N0) echo "N0: no descheduler" > "$OUTPUT_DIR/descheduler.log" ;;
  R0) "$SHARED_ROOT/scripts/run-descheduler.sh" "$ROOT/policies/r0-rdc2.yaml" "$OUTPUT_DIR" ;;
  R1) "$SHARED_ROOT/scripts/run-descheduler.sh" "$ROOT/policies/r1-rdc2-actual.yaml" "$OUTPUT_DIR" ;;
  H0) "$SHARED_ROOT/scripts/run-descheduler.sh" "$ROOT/policies/h0-hnu.yaml" "$OUTPUT_DIR" ;;
  H1) "$SHARED_ROOT/scripts/run-descheduler.sh" "$ROOT/policies/h1-hnu-actual.yaml" "$OUTPUT_DIR" ;;
esac

log "recording post-event window for ${POST_EVENT_SECONDS}s"
sleep "$POST_EVENT_SECONDS"
"$SHARED_ROOT/scripts/snapshot.sh" after "$OUTPUT_DIR"
...
python3 "$SHARED_ROOT/scripts/summarize-run.py" "$OUTPUT_DIR" | tee "$OUTPUT_DIR/summary.txt"
log "run complete"
