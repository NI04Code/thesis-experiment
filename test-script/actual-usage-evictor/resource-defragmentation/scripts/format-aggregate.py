...
def main():
    ...
    experiment_root = pathlib.Path(args.root) if args.root else pathlib.Path(__file__).resolve().parents[2]
    if experiment_root.name == "resource-defragmentation":
        experiment_root = experiment_root.parent
    result_root = experiment_root / "results" / "resource-defragmentation" / args.pattern / args.resource
