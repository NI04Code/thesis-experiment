...
```bash
cd descheduler-custom/test-script/actual-usage-evictor
...
Output akan disimpan di `results/resource-defragmentation/walkthrough/r0-load/<timestamp>/`.
...
```bash
./shared/scripts/setup-layout.sh
```
...
```bash
OUTPUT_DIR="results/resource-defragmentation/walkthrough/r0-load"
mkdir -p "$OUTPUT_DIR"

./shared/scripts/snapshot.sh before "$OUTPUT_DIR"
...
k6 run \
  ...
  shared/k6/api-load.js > "$OUTPUT_DIR/api-load.log" 2>&1 &
...
```bash
# Apply R0 policy
./shared/scripts/run-descheduler.sh resource-defragmentation/policies/r0-rdc2.yaml "$OUTPUT_DIR"
...
./shared/scripts/snapshot.sh after "$OUTPUT_DIR"
