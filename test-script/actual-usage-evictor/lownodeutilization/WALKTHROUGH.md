...
each run, `lownodeutilization/scripts/setup-scheduler.sh` checks the active scheduler:
...
```bash
./lownodeutilization/scripts/setup-scheduler.sh
```
...
```text
results/lownodeutilization/l0-load/<timestamp>/
results/lownodeutilization/l1-load/<timestamp>/
```
