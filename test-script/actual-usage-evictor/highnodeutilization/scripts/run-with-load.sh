...
EXPERIMENT_ROOT="$(cd "$HNU_ROOT/.." && pwd)"
SHARED_ROOT="$EXPERIMENT_ROOT/shared"
NS="actual-usage-exp"
TIMESTAMP="$(date +%Y%m%d-%H%M%S)"
OUTPUT_DIR="$EXPERIMENT_ROOT/results/highnodeutilization/${SYSTEM,,}-load/$TIMESTAMP"
mkdir -p "$OUTPUT_DIR"
...
python3 "$SHARED_ROOT/scripts/watch-pods.py" \
...
"$SHARED_ROOT/scripts/run-descheduler.sh" "$policy" "$OUTPUT_DIR"
...
python3 "$SHARED_ROOT/scripts/summarize-run.py" "$OUTPUT_DIR" \
