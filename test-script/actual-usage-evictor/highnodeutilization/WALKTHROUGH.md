...
Run from `test-script/actual-usage-evictor`. The scripts default to:
...
1. Installs the NodeResourcesFit/MostAllocated configuration from
   `highnodeutilization/scheduler/most-allocated-config.yaml` as
...
```bash
./highnodeutilization/scripts/setup-scheduler.sh
...
```text
results/highnodeutilization/h0-load/<timestamp>/
results/highnodeutilization/h1-load/<timestamp>/
```
...
grep -E 'Evicted pod|blocking eviction|ActualUsageEvictor' \
  results/highnodeutilization/h1-load/<timestamp>/descheduler.log

cat results/highnodeutilization/h0-load/<timestamp>/result-validation.json
cat results/highnodeutilization/h1-load/<timestamp>/result-validation.json
```
...
```bash
API_RPS=12 API_CPU_UNITS=1200 ./highnodeutilization/scripts/run-h1-with-load.sh
```
